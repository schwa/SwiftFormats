...
//        XCTAssertEqual(try SIMDParseStrategy(scalarStrategy: FloatingPointFormatStyle<Float>.number.parseStrategy, mappingStyle: false).parse("0, 1, 2"), vector)
        XCTAssertEqual(try VectorParseStrategy(scalarStrategy: FloatingPointFormatStyle<Float>.number.parseStrategy, compositeStyle: .mapping).parse("x: 0, y: 1, z: 2"), vector)
    }
}
